...
% the double quotes outside the single quotes allow the Bash variable to be referenced.
    fprintf(fid,'python -c ''import sys; sys.path.insert(0,"functions_PACE"); import misvm_PACE; misvm_PACE.misvm_from_dependency_string("dependencies/est_label_misvm_dependencies/dependency''"${PBS_ARRAYID}"''.pickle"); f = open("misvmPBS%dOut/''"${PBS_ARRAYID}"''","w+"); f.close()''\n',ii);
    fclose(fid);
end
